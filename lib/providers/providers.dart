...
class SettingsController extends AsyncNotifier<AppSettings> {
  ...
  Future<void> save({required String apiKey, required String vocabulary}) async {
    final store = SecureSettings();
...
final dayLogsProvider =
    AsyncNotifierProvider<DayLogsController, List<LogEntry>>(DayLogsController.new);
...
final appSettingsProvider = AsyncNotifierProvider<AppSettingsController,
    AppSettingsData>(AppSettingsController.new);
...
final monthTotalsProvider =
    FutureProvider.family<Map<String, LogTotals>, DateTime>(
        (ref, month) async {
  final db = await ref.watch(databaseProvider.future);
...
final streakProvider = FutureProvider<Streaks>((ref) async {
  ...
  return computeStreaks(perDay, today, maintenance);
});
...
final coachDraftProvider = NotifierProvider<CoachDraft, String?>(CoachDraft.new);
